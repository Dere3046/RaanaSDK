...
extern "C" {
    #[link_name = "rs_rb_left"]
    fn rs_rb_left(node: *const bindings::rb_node) -> *mut bindings::rb_node;
    #[link_name = "rs_rb_right"]
    fn rs_rb_right(node: *const bindings::rb_node) -> *mut bindings::rb_node;
    #[link_name = "rs_rb_left_ptr"]
    fn rs_rb_left_ptr(node: *mut bindings::rb_node) -> *mut *mut bindings::rb_node;
    #[link_name = "rs_rb_right_ptr"]
    fn rs_rb_right_ptr(node: *mut bindings::rb_node) -> *mut *mut bindings::rb_node;
    #[link_name = "rs_rb_root_node"]
    fn rs_rb_root_node(root: *const bindings::rb_root) -> *mut bindings::rb_node;
    #[link_name = "rs_rb_root_node_ptr"]
    fn rs_rb_root_node_ptr(root: *mut bindings::rb_root) -> *mut *mut bindings::rb_node;
}
    ...
    K: Ord,
{
    ...
    fn raw_entry(&mut self, key: &K) -> RawEntry<'_, K, V> {
        ...
        let mut child_field_of_parent: &mut *mut bindings::rb_node =
            ...
            unsafe { &mut *rs_rb_root_node_ptr(&mut (*raw_self).root) };
        while !(*child_field_of_parent).is_null() {
            ...
            match key.cmp(unsafe { &(*node).key }) {
                ...
                Ordering::Less => child_field_of_parent = unsafe { &mut *rs_rb_left_ptr(curr) },
                // SAFETY: `curr` is a non-null node so it is valid by the type invariants.
                Ordering::Greater => child_field_of_parent = unsafe { &mut *rs_rb_right_ptr(curr) },
                Ordering::Equal => {
    ...
    pub fn get(&self, key: &K) -> Option<&V> {
        let mut node = unsafe { rs_rb_root_node(&self.root) };
        while !node.is_null() {
            ...
            node = match key.cmp(unsafe { &(*this).key }) {
                // SAFETY: `node` is a non-null node so it is valid by the type invariants.
                Ordering::Less => unsafe { rs_rb_left(node) },
                // SAFETY: `node` is a non-null node so it is valid by the type invariants.
                Ordering::Greater => unsafe { rs_rb_right(node) },
                // SAFETY: `node` is a non-null node so it is valid by the type invariants.
        ...
        K: Ord,
    {
        ...
        while !node.is_null() {
            ...
            // SAFETY: `node` is a non-null node so it is valid by the type invariants.
            let left_child = unsafe { rs_rb_left(node) };
            // SAFETY: `node` is a non-null node so it is valid by the type invariants.
            let right_child = unsafe { rs_rb_right(node) };
