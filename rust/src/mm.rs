...
// Local FFI declarations for the mm helper shims.
//
// These mirror the `rust_helper_*` / `rs_*` symbols provided by the C side so that this module
// does not depend on the exact set of helpers exposed through `bindings`.
extern "C" {
    #[link_name = "rust_helper_mmgrab"]
    fn mmgrab(mm: *mut bindings::mm_struct);
    #[link_name = "rust_helper_mmdrop"]
    fn mmdrop(mm: *mut bindings::mm_struct);
    #[link_name = "rust_helper_mmget"]
    fn mmget(mm: *mut bindings::mm_struct);
    #[link_name = "rust_helper_mmget_not_zero"]
    fn mmget_not_zero(mm: *mut bindings::mm_struct) -> bool;
    #[link_name = "rs_mmput"]
    fn mmput(mm: *mut bindings::mm_struct);
    #[link_name = "rs_mmput_async"]
    fn mmput_async(mm: *mut bindings::mm_struct);

    #[link_name = "rust_helper_mmap_read_lock"]
    fn mmap_read_lock(mm: *mut bindings::mm_struct);
    #[link_name = "rust_helper_mmap_read_trylock"]
    fn mmap_read_trylock(mm: *mut bindings::mm_struct) -> bool;
    #[link_name = "rust_helper_mmap_read_unlock"]
    fn mmap_read_unlock(mm: *mut bindings::mm_struct);

    #[link_name = "rust_helper_vma_lookup"]
    fn vma_lookup(mm: *mut bindings::mm_struct, addr: usize) -> *mut bindings::vm_area_struct;
    #[link_name = "rust_helper_vma_end_read"]
    fn vma_end_read(vma: *mut bindings::vm_area_struct);

    #[cfg(CONFIG_PER_VMA_LOCK)]
    #[link_name = "rust_helper_lock_vma_under_rcu"]
    fn lock_vma_under_rcu(
...
unsafe impl AlwaysRefCounted for Mm {
    ...
    fn inc_ref(&self) {
        ...
        unsafe { mmgrab(self.as_raw()) };
    }

    #[inline]
    unsafe fn dec_ref(obj: NonNull<Self>) {
        // SAFETY: The caller is giving up their refcount.
        unsafe { mmdrop(obj.cast().as_ptr()) };
    }
}
...
unsafe impl AlwaysRefCounted for MmWithUser {
    ...
    fn inc_ref(&self) {
        // SAFETY: The pointer is valid since self is a reference.
        unsafe { mmget(self.as_raw()) };
    }

    #[inline]
    unsafe fn dec_ref(obj: NonNull<Self>) {
        // SAFETY: The caller is giving up their refcount.
        unsafe { mmput(obj.cast().as_ptr()) };
    }
}
...
unsafe impl AlwaysRefCounted for MmWithUserAsync {
    ...
    unsafe fn dec_ref(obj: NonNull<Self>) {
        // SAFETY: The caller is giving up their refcount.
        unsafe { mmput_async(obj.cast().as_ptr()) };
    }
}
...
impl Mm {
    ...
    pub fn mmget_not_zero(&self) -> Option<ARef<MmWithUser>> {
        // SAFETY: The pointer is valid since self is a reference.
        let success = unsafe { mmget_not_zero(self.as_raw()) };
...
impl MmWithUser {
    ...
    pub fn lock_vma_under_rcu(&self, vma_addr: usize) -> Option<VmaReadGuard<'_>> {
        #[cfg(CONFIG_PER_VMA_LOCK)]
        {
            // SAFETY: Calling `lock_vma_under_rcu` is always okay given an mm where
            // `mm_users` is non-zero.
            let vma = unsafe { lock_vma_under_rcu(self.as_raw(), vma_addr) };
            if !vma.is_null() {
    ...
    pub fn mmap_read_lock(&self) -> MmapReadGuard<'_> {
        // SAFETY: The pointer is valid since self is a reference.
        unsafe { mmap_read_lock(self.as_raw()) };
    ...
    pub fn mmap_read_trylock(&self) -> Option<MmapReadGuard<'_>> {
        // SAFETY: The pointer is valid since self is a reference.
        let success = unsafe { mmap_read_trylock(self.as_raw()) };
...
impl<'a> MmapReadGuard<'a> {
    ...
    pub fn vma_lookup(&self, vma_addr: usize) -> Option<&virt::VmaRef> {
        ...
        let vma = unsafe { vma_lookup(self.mm.as_raw(), vma_addr) };
...
impl Drop for MmapReadGuard<'_> {
    ...
    fn drop(&mut self) {
        ...
        unsafe { mmap_read_unlock(self.mm.as_raw()) };
    }
}
...
impl Drop for VmaReadGuard<'_> {
    ...
    fn drop(&mut self) {
        // SAFETY: We hold the read lock by the type invariants.
        unsafe { vma_end_read(self.vma.as_ptr()) };
    }
}
