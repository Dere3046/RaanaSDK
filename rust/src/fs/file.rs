...
pub mod flags {
    // `bindings` deliberately keeps `struct file` opaque and does not expose the
    // UAPI `O_*` constants. These are the arm64/asm-generic values used by GKI.
    /// File is opened in append mode.
    pub const O_APPEND: u32 = 0o20000;
...
// Local FFI accessors for the opaque `bindings::file` and the file/fd APIs that
// are not exposed through `bindings`.
//
// `rs_file_*` field accessors are provided by the C shim; `rs_fget`, `rs_fput`,
// `rs_fd_install`, `rs_get_unused_fd_flags` and `rs_put_unused_fd` are the
// runtime-resolved kernel function wrappers used by this crate.
extern "C" {
    #[link_name = "rs_file_private_data"]
    fn rs_file_private_data(file: *mut bindings::file) -> *mut crate::ffi::c_void;
    #[link_name = "rs_file_cred"]
    fn rs_file_cred(file: *mut bindings::file) -> *const bindings::cred;
    #[link_name = "rs_file_flags"]
    fn rs_file_flags(file: *mut bindings::file) -> u32;
    #[link_name = "rs_fget"]
    fn rs_fget(fd: u32) -> *mut bindings::file;
    #[link_name = "rs_fput"]
    fn rs_fput(file: *mut bindings::file);
    #[link_name = "rs_get_unused_fd_flags"]
    fn rs_get_unused_fd_flags(flags: u32) -> i32;
    #[link_name = "rs_put_unused_fd"]
    fn rs_put_unused_fd(fd: u32);
    #[link_name = "rs_fd_install"]
    fn rs_fd_install(fd: u32, file: *mut bindings::file);
}
...
unsafe impl AlwaysRefCounted for File {
    ...
    unsafe fn dec_ref(obj: ptr::NonNull<File>) {
        ...
        unsafe { rs_fput(obj.cast().as_ptr()) }
    }
}
...
unsafe impl AlwaysRefCounted for LocalFile {
    ...
    unsafe fn dec_ref(obj: ptr::NonNull<LocalFile>) {
        // SAFETY: To call this method, the caller passes us ownership of a normal refcount, so we
        // may drop it. The cast is okay since `LocalFile` has the same representation as `struct file`.
        unsafe { rs_fput(obj.cast().as_ptr()) }
    }
}
...
impl LocalFile {
    ...
    pub fn fget(fd: u32) -> Result<ARef<LocalFile>, BadFdError> {
        ...
        let ptr = ptr::NonNull::new(unsafe { rs_fget(fd) }).ok_or(BadFdError)?;

        // SAFETY: `rs_fget` created a refcount, and we pass ownership of it to the `ARef`.
        //
    ...
    pub fn cred(&self) -> &Credential {
        ...
        let ptr = unsafe { rs_file_cred(self.as_ptr()) };
    ...
    pub fn flags(&self) -> u32 {
        // SAFETY: The file is valid because the shared reference guarantees a nonzero refcount.
        unsafe { rs_file_flags(self.as_ptr()) }
    }

    /// Returns the file's private data.
    ///
    /// The caller is responsible for interpreting the pointer according to the file's fops
    /// contract.
    #[inline]
    pub fn private_data(&self) -> *mut crate::ffi::c_void {
        // SAFETY: The shared reference guarantees that `self.as_ptr()` points at a live file for
        // the duration of this call.
        unsafe { rs_file_private_data(self.as_ptr()) }
    }
}
...
impl core::ops::Deref for File {
    ...
    fn deref(&self) -> &LocalFile {
        ...
        // `f_pos_lock` mutex.
        unsafe { LocalFile::from_raw_file(self.as_ptr()) }
    }
}
...
impl FileDescriptorReservation {
    ...
    pub fn get_unused_fd_flags(flags: u32) -> Result<Self> {
        ...
        let fd: i32 = unsafe { rs_get_unused_fd_flags(flags) };
        if fd < 0 {
    ...
    pub fn fd_install(self, file: ARef<File>) {
        ...
        // file position.
        unsafe { rs_fd_install(self.fd, file.as_ptr()) };
...
impl Drop for FileDescriptorReservation {
    fn drop(&mut self) {
        ...
        unsafe { rs_put_unused_fd(self.fd) };
    }
}
