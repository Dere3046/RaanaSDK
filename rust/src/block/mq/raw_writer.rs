...
impl<'a> RawWriter<'a> {
    ...
    /// Create a new `RawWriter` from a C string buffer.
    ///
    /// # Safety
    ///
    /// `ptr` must be valid for reads and writes for `len` bytes.
    pub(crate) unsafe fn from_raw_parts<'b>(
        ptr: *mut crate::ffi::c_char,
        len: usize,
    ) -> Result<RawWriter<'b>> {
        // SAFETY: The caller guarantees that `ptr` is valid for `len` bytes.
        let buffer = unsafe { core::slice::from_raw_parts_mut(ptr.cast::<u8>(), len) };
        *(buffer.last_mut().ok_or(EINVAL)?) = 0;

        // INVARIANT: We null terminated the buffer above.
        Ok(RawWriter { buffer, pos: 0 })
    }
}
